...
| oriolj (personal) | Oriol's personal Coolify Cloud team (`oriolj-apps-1`, H2A Accountant…) | `homelab/secrets/coolify-oriolj.env` (`COOLIFY_ORIOLJ_API_TOKEN`, `COOLIFY_ORIOLJ_API_URL` = `https://app.coolify.io/api/v1` — the URL already ends in `/api/v1`, do not append it again or every call is a 404), verified 2026-09-20 (deployments + applications reads) | `coolify/coolify-oriolj.pub` — to be created |
...
- `SOURCE_COMMIT` = full 40-char deployed SHA build arg. ⚠️ **Contradictory field data — set `include_source_commit_in_build: true` and stop guessing**: on enachat (2026-08-25) the arg arrived with the toggle `false`; on EnCaSaGo (2026-09-02, GitHub-App source, `ARG SOURCE_COMMIT=dev` in the builder stage) the build read `dev` until the resource was PATCHed `{"include_source_commit_in_build": true}` — the next webhook build then stamped the SHA. The field reads back `null` either way (not serialised), so the only proof is the running app's version. Make the toggle part of every resource's creation body. Feed it to Sentry release / version endpoints. **Never pin a derived value like SENTRY_RELEASE in the UI env** — it freezes stale. Keep such ARGs in a **late, cheap Dockerfile layer** or every deploy busts the apt/pip cache.
- 🔴 **Compose resources: `build: .` — never `build: {context, args}`** (EnaStats, three failed deploys 2026-09-09). With an `args` mapping present, Coolify rewrites the service's `environment` from a list to a map, and a bare magic item (`- SERVICE_FQDN_WEB_8000`) becomes the key `0` → `non-string key in services.web.environment: 0`. The deploy fails, the OLD stack keeps running, and from outside nothing you pushed has any effect — every symptom reads as a code bug. **Before debugging a deploy that "did nothing", read `GET /deployments/applications/{uuid}` (status per commit)**; a `failed` row for your sha is the whole diagnosis. The args block is never needed: Coolify runs `docker compose build --build-arg SOURCE_COMMIT=<sha> …` itself, so a Dockerfile `ARG SOURCE_COMMIT` receives it on its own — bake it into a file in the image (`/app/.source_commit`) and read env-then-file at runtime, because a compose runtime line `SOURCE_COMMIT=${SOURCE_COMMIT:-}` interpolates EMPTY and overrides the image `ENV` (`EnaStats/release.py` is the reference).
...
**The token contains a `|`** (`<id>|<48 chars>`): `source`-ing `coolify.env` runs the tail as a command and every call answers 401 — read it with `grep '^COOLIFY_API_TOKEN=' | cut -d= -f2-`.
...
| Pushes "do nothing": the app runs an old commit, `app_info{version}` never changes, every fix you ship has no effect | The compose deploy is FAILING and the old stack keeps serving — on a compose resource the usual cause is `build: {context, args}` → `non-string key in services.<svc>.environment: 0` (§3) | `GET /deployments/applications/{uuid}` first: a `failed` row per push is the diagnosis; read its log, fix (`build: .`), redeploy |
| Traces/OTLP from a COMPOSE resource never arrive; the app logs `NewConnectionError` on `oj-alloy` (or `ConnectTimeout` on the tailnet IP) while Dockerfile apps on the same host trace fine | Compose services sit on a per-resource network; the agent's alias lives on `coolify` (§2) | `PATCH {"connect_to_docker_network": true}` + deploy; never the host's tailnet IP from a container |
...
| Django migrate-on-boot dies with `cannot CREATE INDEX "<table>…" because it has pending trigger events`; the deploy rolls back (or, worse, the rollback container is gone and the app is down) | A migration mixes `AddField(db_index=True)` / new FKs with a `RunPython` that UPDATEs the same table: Django emits the `CREATE INDEX` statements at the END of the migration (deferred SQL), after the UPDATE left deferred FK-trigger events pending in the same transaction. Works on an empty dev DB, fails on real data (H2A-Accountant: the webhook rollout of `630a168` failed 2026-09-05 22:16 UTC and left the app `exited:unhealthy` with NO container — API 503 for **8 h 21 min** until the fix deployed 06:37; Talaia alerted from 22:18, nobody looked until morning). **Diagnosis without SSH**: the deploy log carries the container's stdout (a `Container logs:` block with the Django traceback, `GET /deployments/{uuid}`) only when the health gate exhausts all 10 attempts with the container still present — in the 22:12 run the container vanished at attempt 6 (`No such object`) and nothing was captured; `GET /applications/{uuid}/logs` answers 400 `Application is not running` once it is down. So re-trigger the deploy (`POST /deploy?uuid=…`) and read the block; and reproduce at the deployed commit against a throwaway Postgres WITH a row in the affected table — a from-scratch migrate passes and proves nothing (verified both ways 2026-09-06) | Move the `RunPython` into its own migration (next number, depends on the schema one; idempotent UPDATEs so dev DBs that ran the old file are fine) — or, when a new migration number would collide with parallel work, `schema_editor.execute("SET CONSTRAINTS ALL IMMEDIATE")` inside the RunPython before the UPDATEs (the pattern `invoices.0016` already used; both fixes verified on the reproduction 2026-09-06). Rehearse `manage.py migrate` against a `pg_restore` of the latest prod dump before pushing — that is what the R2 backups are for |
...
| Pushover/mail «`<app>` stopped — restart limit reached (N/10)» on an app that had NOT been restarting; `docker ps -a` shows every container `Exited (0)` at the same second with `hasBeenManuallyStopped=true` in the dockerd journal | **Coolify ≥ 4.3.19 (rolled out to Cloud 2026-09-15) added a Docker restart limit: `max_restart_count` default **10** on every application/service/database, enforced from the Sentinel push (`PushServerUpdateJob`): max of the containers' `restart_count` ≥ limit → `StopApplication` + `RestartLimitReached` notification.** The count it compares is Coolify's OWN accumulated counter (never reset before 4.3.19 — `last_restart_at` months old) and Docker's lifetime `RestartCount` per container, so a resource whose containers date from long ago (infra-monitoring's Nov-2025 legacy stack: Coolify said 42, Docker 11 from the April boot, uptime 144 days) is stopped the minute the new Sentinel (1.0.1, mandatory) reports it — nothing crashed. A successful deploy/restart resets Coolify's counter to 0, but the recreated containers' Docker counter is what gets reported next, and a plain `docker restart` keeps it | `PATCH /applications/{uuid} {"max_restart_count": 0}` (off) or a value above the stale count, THEN `POST …/restart` (`coolify-restart.sh`) — a restart alone is re-stopped on the next push if the limit still bites. Sweep the other teams after any Coolify upgrade: `GET /applications` → `restart_count` vs `max_restart_count` (2026-09-15: only the legacy stack was ≥ 10; next highest was 1). The limit is a fine crash-loop guard on fresh apps — leave the default there. (That stack was decommissioned outright later the same day: `DELETE /applications/{uuid}?delete_configurations=true&delete_volumes=true&docker_cleanup=true` removed the resource, containers, images and `/data/coolify/applications/<uuid>` but **left the three named volumes on the host** — after any Coolify delete, `docker volume ls --filter name=<uuid>` and `rm` them yourself) |
