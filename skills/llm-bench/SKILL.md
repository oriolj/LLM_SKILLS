...
- **Panotxa / NutriLens (vision, google-genai direct)** — reference implementation
  ...
  in `backend/docs/plans/`. Features `dish_photo` (37 items, 15 with the
  nutritionist's DQ) and `menu_extraction` (12); replay through `inspect.unwrap`
  of the `@observe` function with a `model=` seam; a `BenchGeminiClient` routes
  `gemini-*` to the native SDK (thinking via `thinking_budget=0` / `thinking_level`)
  and everything else to OpenRouter, returning a genai-shaped response so the
  production JSON path runs unchanged. First reading: production flash-lite kept
  (best DQ agreement r 0.85, 3.8 s), `gemini-3.1-pro` the only clear quality gain
  (hallucinated specifics 11 % → 2 %) at 9.4 s, gpt-6-luna worse on vision,
  `gemini-3.8-flash` better on Recon recall but with broken boxes. Whole LLM bill
  ≈ $4/month: there, a model choice is latency and quality, not money. Next:
  text features (`meal_quality`, `daily_insight`, chat), `response_schema`.
