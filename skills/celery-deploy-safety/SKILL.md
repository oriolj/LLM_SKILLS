...
### Three shapes that looked idempotent and were not (EnaPost/EnaScribe, Codex review 2026-08-30)

- **Status flip, then fallible work, in the request.** `publish()` saved `status=PUBLISHED`, then made synchronous LLM translation calls, then created the Delivery rows and dispatched the async channels — all inside the staff request. A provider timeout (or gunicorn's 60 s) left the post public and half fanned-out, and the view's `if status != PUBLISHED` guard made it unrepairable. Shape that works: flip the status in one short `transaction.atomic()`, then `.delay()` ONE `fanout_task` (`autoretry_for=(Exception,)`, backoff) whose every step is idempotent (`get_or_create` per (row, channel); sub-tasks that skip on their own Delivery row). The request never carries provider calls; a retry resumes. Test it by making the first attempt raise and asserting one Delivery per channel after the retry — in eager mode the autoretry surfaces as `celery.exceptions.Retry`, so `pytest.raises((RuntimeError, Retry))`.
