---
name: secrets-in-git
description: "Manage credentials in private personal repositories using age-passphrase or sops encryption. Use when adding, finding, rotating, encrypting, committing or deploying API keys, tokens and passwords; reviewing secrets directories or ciphertext; exporting SaaS environment variables; creating secrets Makefile or Ansible workflows; recovering credentials after cloning; or diagnosing API 401/403 failures, including Cloudflare client-IP restrictions. Covers scope separation, external references for company repositories, safe local parsing, catalog and USER_TODO handoffs, single-file encryption, freshness checks and provider-to-runtime rotation. Never store plaintext or encrypted credentials in professional repositories."
---
