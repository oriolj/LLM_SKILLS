...
- **[pydantic-ai-langfuse](skills/pydantic-ai-langfuse/SKILL.md)** — Implement LLM features in Python with PydanticAI + Langfuse observability: agent/prompt builder patterns, PydanticAI 2.x breaking changes (`google:` provider rename, `Agent.instrument_all()`), OTel→Langfuse init that no-ops without keys, `@observe` capture rules (Django ORM OOM trap), **cost attribution** (a tenant `user_id` on every trace — first-class field, not metadata; `propagate_attributes` in v4 vs `update_trace` in v3; `cost_details` keyed `total`, never `total_cost`; the read-only API script that buckets spend by trace name × userId to name the untagged code path), TestModel/FunctionModel testing with `ALLOW_MODEL_REQUESTS=False`, rate-limit-safe batch scoring, user-steerable judge prompts, and SPA-aware website fetching for LLM input.
- **[zero-cookies](skills/zero-cookies/SKILL.md)** — Engineer and audit cookie-free public surfaces: state via localStorage/server-side tokens instead of cookies so no consent banner is ever needed. The localStorage-is-also-regulated trap (ePrivacy art. 5(3) exempts NECESSITY, not the technology), Django cookie-emission gotchas (sessions, CSRF, messages, i18n), the staff-backoffice exception, third-party embed swaps (youtube-nocookie, self-hosted fonts, cookieless analytics), audit greps + the `Set-Cookie`-absence regression test per public route. Build/enforcement counterpart to **eu-law** (which owns the legal decision tree).
...
- **[wp-plugin-dev](skills/wp-plugin-dev/SKILL.md)** — Build, version and distribute WordPress plugins, especially private/client plugins shipped as a zip (no wordpress.org listing): plugin identity rules (stable folder + main file, the one-folder zip layout), the WP 5.5 "Replace current with uploaded" update flow that upgrades in place without duplicating, the WP 5.8 `Update URI` header against wp.org same-slug hijacking, self-hosted auto-updates (plugin-update-checker or hand-rolled `update_plugins_{hostname}` + `plugins_api` with transient caching), the security five-step for every handler (capability, nonce, sanitize, prepared SQL, escape), lifecycle hooks vs update-time migrations, and a release checklist.
- **[auth-session-resilience](skills/auth-session-resilience/SKILL.md)** — Diagnose and fix "users keep getting logged out" in SPA + JWT apps, and wire login/token-refresh right from day one: the dj-rest-auth `JWT_AUTH_HTTPONLY` empty-refresh trap (`refresh: ""` in the login body → forced re-login every access-token lifetime), never treating network errors as session expiry (offline/wake-from-sleep must not log out), single-flight refresh under token rotation + blacklisting (parallel 401s blacklist each other), cross-tab rotation races, the boot-time fetchUser token-wipe trap, server-side token-contract pytest, and the Playwright verification playbook (tampered access token + parallel requests, route-abort offline simulation, dead-session redirect).
- **[api-idempotency](skills/api-idempotency/SKILL.md)** — Make unsafe API writes idempotent so retries, flaky connections, and offline-queue replays can't create duplicate rows: the client-generated idempotency-key pattern (Stripe-style), the mint-once-per-intent rule, DB unique constraints + race handling (Django IntegrityError savepoints, Go/Postgres `ON CONFLICT`), the retry-middleware key-minting trap (interceptors re-run per attempt), transient-vs-permanent retry classification, and how to audit a codebase for the "infra exists but a path skips it" gap.
- **[prod-db-sync](skills/prod-db-sync/SKILL.md)** — Pull a production Postgres from a Coolify/docker host into local Docker Compose, safely: `pg_dump` inside the prod container over SSH (no prod creds locally), local-gzip + completion-marker dump verification (the `grep -q` SIGPIPE trap), temp-DB restore + swap so failures never leave local empty, container auto-detection on multi-stack hosts (postgres image **and** `POSTGRES_DB` — compose leaks the env var into every service), gitignored `.envs/.prod-sync` config, post-restore migrations, and the real-API-keys-in-synced-data warning. Ships `sync-db-prod.template.sh` to adapt.
